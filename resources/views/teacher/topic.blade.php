...
<div class="mx-auto max-w-2xl px-4 py-16 sm:px-6 sm:py-24 lg:max-w-7xl lg:px-8">
    ...
    <article class="rounded-lg border border-gray-100 bg-white p-4 shadow-xs transition hover:shadow-lg sm:p-6 mb-5">
        <div class="flex items-center justify-between">
            ...
            <div class="flex items-center relative">
                ...
                <div id="dropdownMenu" class="hidden absolute right-0 mt-2 w-25 bg-white rounded-lg shadow-lg">
                    <a href="{{ route('topic.edit', ['category' => $category->id, 'kelas' => $kelas->id, 'topic' => $topic->id]) }}" class="w-full text-left block px-4 py-2 text-gray-600 hover:bg-blue-500 hover:text-white text-sm font-bold cursor-pointer hover:rounded-lg">Edit</a>
                    <form method="POST" action="{{ route('topic.destroy', ['category' => $category->id, 'kelas' => $kelas->id, 'topic' => $topic->id]) }}">
                        @method('DELETE')
                        @csrf
                        <button type="submit" onclick="return confirm('Anda akan menghapus materi ini?')"
        ...
        <div class="items-end justify-end">
            ...
            <!-- hidden reply form -->
            <form id="replyFormTop" action="{{ route('topic.comment.store', $topic->id) }}" method="POST" class="mt-3 hidden">
                @csrf
                <!-- Trix editor -->
                <div>
    ...
    <!-- comment list -->
    <h3 class="text-md text-gray-500 font-semibold mt-6 mb-3">Komentar</h3>
    ...
    <article class="rounded-lg border border-gray-100 bg-white p-4 shadow-sm mb-3">
        <div class="flex items-center justify-between">
            ...
            <div class="flex items-center relative ml-auto">
                ...
                <div id="dropdownMenu-{{ $comment->id }}" class="hidden absolute right-0 mt-2 w-25 bg-white rounded-lg shadow-lg">
                    <form method="POST" action="{{ route('topic.comment.destroy', [$topic->id, $comment->id]) }}">
                        @method('DELETE')
                        @csrf
        ...
        <form id="replyForm-{{ $comment->id }}" action="{{ route('topic.comment.store', $topic->id) }}" method="POST" class="mt-3 hidden">
            @csrf
                ...
                <div class="text-sm flex items-center justify-between">
                    ...
                    <div class="flex items-center relative ml-auto">
                        ...
                        <div id="dropdownMenu-{{ $reply->id }}" class="hidden absolute right-0 mt-2 w-25 bg-white rounded-lg shadow-lg">
                            <form method="POST" action="{{ route('topic.comment.destroy', [$topic->id, $reply->id]) }}">
                                @method('DELETE')
                                @csrf
                                <button type="submit" onclick="return confirm('Anda akan menghapus komentar ini?')"
                                    class="w-full text-left block px-4 py-2 text-gray-600 hover:bg-red-500 hover:text-white text-sm font-bold cursor-pointer hover:rounded-lg">
