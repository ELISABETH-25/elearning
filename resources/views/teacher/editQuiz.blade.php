...
<div class="w-full max-w-sm">
    ...
    <div class="flex items-center justify-between pb-5 mb-4">
        <span aria-hidden="true" class="pb-5 block transition-all group-hover:ms-0.5 rtl:rotate-180">
            <a href="{{ route('teacher.quiz.show', [$category->id, $kelas->id, $quiz->id]) }}" class="font-semibold hover:text-red-500">&larr; Back</a>
        </span>
        <div class="flex flex-col">
            <h2 class="mb-2 text-2xl font-bold">Edit Quiz</h2>
            <p class="italic">{{ $category->name }}</p>
        </div>
    </div>


    <form action="{{ route('quiz.update', [$category->id, $kelas->id, $quiz->id]) }}" method="POST" class="space-y-4">
        @method('PUT')
        @csrf
        ...
        <div>
            ...
            <select id="topic_id" name="topic_id" required
                ...
                @php
                $isCurrent = $quiz->topic_id == $topic->id;
                $isTakenByAnotherQuiz = $topic->quiz && !$isCurrent;
                @endphp
                <option
                    value="{{ $topic->id }}"
                    {{ $isCurrent ? 'selected' : '' }}
                    {{ $isTakenByAnotherQuiz ? 'disabled' : '' }}>
                    {{ $topic->judul }} {{ $isTakenByAnotherQuiz ? '(Materi ini sudah punya quiz)' : '' }}
                </option>
                @endforeach
                @endforeach
            </select>
        ...
        <div>
            ...
            <input id="body" type="hidden" name="body" value="{{ old('body', $quiz->body) }}">
            @error('body')
        ...
        <div>
            ...
            <input type="date" id="deadline" name="deadline" required value="{{ old('deadline', $quiz->deadline ? \Carbon\Carbon::parse($quiz->deadline)->format('Y-m-d') : '') }}"
                class="mt-1 w-full rounded-md border border-gray-300 px-3 py-2 text-sm text-gray-700 focus:border-indigo-500 focus:ring-indigo-500 focus:outline-none
