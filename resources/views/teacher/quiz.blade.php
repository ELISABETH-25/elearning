...
<div class="mx-auto max-w-2xl px-4 py-16 sm:px-6 sm:py-24 lg:max-w-7xl lg:px-8">
    ...
    <div class="flex items-center justify-between pb-5 mb-4">
        ...
        <a href="{{ route('teacher.category.show', [$category->id, $kelas->id]) }}" class="font-semibold hover:text-blue-700">&larr; Back</a>
        </span>
        <div class="flex flex-col">
            <h2 class="mb-2 text-lg font-medium">Quiz: <strong> {{ $quiz->judul }} </strong></h2>
        </div>
    </div>
    ...
    <article class="rounded-lg border border-gray-100 bg-white p-4 shadow-xs transition hover:shadow-lg sm:p-6 mb-5">
        <div class="flex items-center justify-between">
            <span class="flex rounded-sm bg-blue-600 p-2 text-white">
                ...
                <h3 class="px-3 text-white font-bold">{{ $quiz->teacher_name }}</h3>
            </span>

            <!-- Right section: Dropdown -->
            <div class="flex items-center relative">
                <button id="dropdownToggle" class="cursor-pointer flex items-center text-gray-700 hover:text-gray-900 focus:outline-none">
                ...
                <!-- Dropdown menu -->
                <div id="dropdownMenu" class="hidden absolute right-0 mt-2 w-25 bg-white rounded-lg shadow-lg">
                    <a href="{{ route('quiz.edit', [$category->id, $kelas->id, $quiz->id]) }}" class="w-full text-left block px-4 py-2 text-gray-600 hover:bg-blue-500 hover:text-white text-sm font-bold cursor-pointer hover:rounded-lg">Edit</a>
                    <form method="POST" action="{{ route('quiz.destroy', [$category->id, $kelas->id, $quiz->id]) }}">
                        @method('DELETE')
                        @csrf
                        <button type="submit" onclick="return confirm('Anda akan menghapus quiz ini?')"
        ...
        <div class="items-end justify-end">
            ...
            <!-- hidden reply form -->
            <form id="replyFormTop" action="{{ route('quiz.comment.store', $quiz->id) }}" method="POST" class="mt-3 hidden">
                @csrf
                <!-- Trix editor -->
                <div>
    ...
    <!-- comment list -->
    <h3 class="text-md text-gray-500 mt-6 mb-3">Komentar</h3>

    @forelse($quiz->comments as $comment)
    <article class="rounded-lg border border-gray-100 bg-white p-4 shadow-sm mb-3">
        <div class="flex items-center justify-between">
            ...
            <div class="flex items-center relative ml-auto">
                ...
                <div id="dropdownMenu-{{ $comment->id }}" class="hidden absolute right-0 mt-2 w-25 bg-white rounded-lg shadow-lg">
                    <form method="POST" action="{{ route('quiz.comment.destroy', [$quiz->id, $comment->id]) }}">
                        @method('DELETE')
                        @csrf
        ...
        <form id="replyForm-{{ $comment->id }}" action="{{ route('quiz.comment.store', $quiz->id) }}" method="POST" class="mt-3 hidden">
            @csrf
                ...
                <div class="text-sm flex items-center justify-between">
                    ...
                    <div class="flex items-center relative ml-auto">
                        ...
                        <div id="dropdownMenu-{{ $reply->id }}" class="hidden absolute right-0 mt-2 w-25 bg-white rounded-lg shadow-lg">
                            <form method="POST" action="{{ route('quiz.comment.destroy', [$quiz->id, $reply->id]) }}">
                                @method('DELETE')
                                @csrf
                                <button type="submit" onclick="return confirm('Anda akan menghapus komentar ini?')"
                                    class="w-full text-left block px-4 py-2 text-gray-600 hover:bg-red-500 hover:text-white text-sm font-bold cursor-pointer hover:rounded-lg">
