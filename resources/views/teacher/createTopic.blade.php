...
<div class="w-full max-w-sm">
    ...
    <div class="flex items-center justify-between pb-5 mb-4">
        ...
        <div class="flex flex-col">
            <h2 class="mb-2 text-2xl font-bold">Tambah Materi</h2>
            <p class="italic">{{ $category->name }}</p>
        </div>
    </div>

    <form action="{{ route('topic.store', ['category' => $category->id, 'kelas' => $kelas->id]) }}" method="POST" class="space-y-4">
        @csrf

        <!-- Judul Materi -->
        <div>
            <label for="judul" class="block text-sm font-medium text-gray-700">Judul Materi</label>
            <input type="text" id="judul" name="judul" required autofocus value="{{ old('judul') }}"
