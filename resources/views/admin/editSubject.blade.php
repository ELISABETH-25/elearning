...
<div class="w-full max-w-sm">
    ...
    <div class="flex items-center justify-between pb-5 mb-4">
        <span aria-hidden="true" class="pb-5 block transition-all group-hover:ms-0.5 rtl:rotate-180">
            <a href="{{ route('subject.view', $subject->id) }}" class="font-semibold hover:text-red-500">&larr; Back</a>
        </span>
        <div class="flex flex-col">
            <h2 class="mb-2 text-2xl font-bold">Edit: {{ $section }}</h2>
        </div>
    </div>
    ...
    <form action="{{ route('subject.update', $subject->id) }}" method="POST" class="space-y-4">
        ...
        <!-- multi select Kelas -->
          <div>
            ...
            <select id="kelas_id" name="kelas_id[]" multiple required
                class="my-1 h-49 w-full rounded-md border border-gray-300 px-3 py-2 text-sm text-gray-700 focus:border-indigo-500 focus:ring-indigo-500 focus:outline-none">
                @foreach ($kelas as $class)
                    <option class="py-1 px-2" value="{{ $class->id }}" {{ in_array($class->id, old('kelas_id', $subject->kelasDirect->pluck('id')->toArray())) ? 'selected' : '' }}>
