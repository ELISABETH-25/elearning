            ...
            <table class="min-w-full divide-y divide-gray-200 text-xs">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-4 py-2 text-left font-medium text-gray-500 uppercase tracking-wider">Tingkat</th>
                        <th class="px-4 py-2 text-left font-medium text-gray-500 uppercase tracking-wider">Kode Kelas</th>
                        <th class="px-4 py-2 text-left font-medium text-gray-500 uppercase tracking-wider">Program Studi</th>
                        <th class="px-4 py-2 text-left font-medium text-gray-500 uppercase tracking-wider">Jumlah Siswa</th>
                        <th class="px-4 py-2 text-left font-medium text-gray-500 uppercase tracking-wider">Jumlah Mapel</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200 text-xs">
                    ...
                    <tr class="hover:bg-gray-100">
                        ...
                        <td class="px-4 py-2">{{ $class->tingkat }}</td>
                        <td class="px-4 py-2">{{ $class->name }}</td>
                        <td class="px-4 py-2">{{ $class->prodi->name }}</td>
                        <td class="px-4 py-2">{{ $class->students_count }}</td>
                        <td class="px-4 py-2">{{ $class->categories_direct_count }}</td>
                        <td class="px-1 py-2">
                            <div class="flex items-center justify-between gap-3">
                                <a href="{{ route('kelas.view', $class->id) }}" class="cursor-pointer">
                                    <img class="w-5 h-5 transition-transform hover:scale-125" src="{{ asset('images/view.png') }}" alt="view" />
                                </a>
                                <a href="{{ route('kelas.edit', $class->id) }}" class="cursor-pointer">
                                    <img class="w-4 h-4 transition-transform hover:scale-125" src="{{ asset('images/edit.png') }}" alt="edit" />
                                </a>
                                <form method="POST" action="{{ route('kelas.destroy', $class->id) }}">
                                    @method('DELETE')
                                    @csrf
